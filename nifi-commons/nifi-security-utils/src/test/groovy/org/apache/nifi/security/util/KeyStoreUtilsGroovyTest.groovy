...
//        boolean keyPasswordIsValid = KeyStoreUtils.isKeyPasswordCorrect(KEYSTORE_FILE.toURI().toURL(), KEYSTORE_TYPE, KEYSTORE_PASSWORD.toCharArray(), KEYSTORE_PASSWORD.reverse().toCharArray())
    ...
    void testShouldValidateTlsConfigAndNewKeystoreTruststoreWithoutParams() {
        // Arrange
        TlsConfiguration tlsConfigParam = new StandardTlsConfiguration(null, TEST_KEYSTORE_PASSWORD, JKS_STORE_TYPE, null, null, null)

        // Act
        TlsConfiguration tlsConfig = KeyStoreUtils.createTlsConfigAndNewKeystoreTruststore(tlsConfigParam)
        final URL ksUrl = getKeystorePathAsUrl(tlsConfig.getKeystorePath())

        boolean keyPasswordIsValid = KeyStoreUtils.isKeyPasswordCorrect(ksUrl, JKS_STORE_TYPE, TEST_KEYSTORE_PASSWORD.toCharArray(), TEST_KEYSTORE_PASSWORD.reverse().toCharArray())

        deleteKeystoreTruststore(tlsConfig)

        // Assert
        assert tlsConfig.getKeystorePath()
        assert tlsConfig.getKeyPassword() == TEST_KEYSTORE_PASSWORD
        assert tlsConfig.getKeystoreType() == KeystoreType.JKS
    ...
    @Test
    void testShouldValidateTlsConfigAndNewTruststoreOnlyWithParams() {
        // Arrange
        TlsConfiguration tlsConfigParam = new StandardTlsConfiguration(null, null, null, null, null, TEST_TRUSTSTORE_PASSWORD, JKS_STORE_TYPE)

        // Act
        final TlsConfiguration tlsConfig = KeyStoreUtils.createTlsConfigAndNewTruststoreOnly(tlsConfigParam)

        final URL tsUrl = getKeystorePathAsUrl(tlsConfig.getTruststorePath())
        boolean truststoreIsValid = KeyStoreUtils.isStoreValid(tsUrl, JKS_STORE_TYPE, TEST_TRUSTSTORE_PASSWORD.toCharArray())

        deleteKeystoreTruststore(tlsConfig)

        // Assert
        assert truststoreIsValid
        assert tlsConfig.getTruststorePath()
        assert tlsConfig.getKeystorePath() == null
        assert tlsConfig.getTruststoreType() == KeystoreType.JKS
        assert tlsConfig.getTruststorePassword() == TEST_TRUSTSTORE_PASSWORD
    }

    @Test
    void testShouldValidateTlsConfigAndNewTruststoreOnlyWithoutParams() {
        // Act
        final TlsConfiguration tlsConfig = KeyStoreUtils.createTlsConfigAndNewTruststoreOnly()

        final URL tsUrl = getKeystorePathAsUrl(tlsConfig.getTruststorePath())
        boolean truststoreIsValid = KeyStoreUtils.isStoreValid(tsUrl, KeystoreType.PKCS12, "".toCharArray())
        ...
        assert truststoreIsValid
        assert tlsConfig.getTruststorePath()
        assert StringUtils.isEmpty(tlsConfig.getTruststorePassword())
        assert tlsConfig.getTruststoreType() == KeystoreType.PKCS12
        assert tlsConfig.getKeystorePath() == null
    }
    ...
    void testShouldValidateTlsConfigWithoutKeyPasswordParam() {
        ...
        final URL ksUrl = getKeystorePathAsUrl(tlsConfig.getKeystorePath())
        boolean keyPasswordIsValid = KeyStoreUtils.isKeyPasswordCorrect(ksUrl, JKS_STORE_TYPE, TEST_KEYSTORE_PASSWORD.toCharArray(), TEST_KEYSTORE_PASSWORD.toCharArray())
